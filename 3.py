# -*- coding: utf-8 -*-
"""
Created on Thu Sep 16 08:41:54 2021
...
def load_input():
	...
	augment_image = cv2.imread('mask.png')
...
if __name__=='__main__':
	...
	while(ret):
		...
		if(matches!=None):
			if(len(matches)>10): #to handle false keypoints
				src_points=np.float32([input_keypoints[m.queryIdx].pt for m in matches]).reshape(-1,1,2)
				dst_points=np.float32([output_keypoints[m.trainIdx].pt for m in matches]).reshape(-1,1,2)
				#Find the homography matrix
				M, mask=cv2.findHomography(src_points, dst_points,cv2.RANSAC,5.0)
				pts=np.float32([[0,0],[0,399],[299,399],[299,0]]).reshape(-1,1,2)
				dst=cv2.perspectiveTransform(pts,M)
				M_aug=cv2.warpPerspective(aug_image,M,(600,450))
				#fill the destination part with black
				frameb=cv2.fillConvexPoly(frame,dst.astype(int),0)
				#addition opertation with Mask Image
				Final=frameb+M_aug
